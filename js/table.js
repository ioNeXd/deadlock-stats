// Encapsulate table logic to avoid globals and expose a single init entrypoint
const TableModule = (() => {
  const state = {
    rows: [],
    sortColumn: null,
  ...
  function updateAriaSort() {
    ...
    for (const [key, el] of Object.entries(headers)) {
      ...
      if (state.sortColumn === key) {
        el.setAttribute('aria-sort', state.sortDirection === 'asc' ? 'ascending' : 'descending');
      } else {
    ...
    for (const row of rows) {
      ...
      if (imgEl) {
        imgEl.addEventListener('error', () => {
          ...
          imgEl.alt = imgAlt ? `${imgAlt} (image unavailable)` : 'Image unavailable';
        });
      }
  ...
  async function renderTable(type) {
    ...
    try {
      ...
      const totalMatches = Number(gameStats[0]?.total_matches) || 0;
      ...
      state.rows = stats.map((stat) => {
        ...
        const imageUrl = type === 'heroes' ? (entity.images && entity.images.icon_image_small) : entity.shop_image;
        return { name: entity.name || 'Unknown', imageUrl: imageUrl || '', winRate, pickRate };
      });
